...
│   ├── processor.py       # test.py의 preprocess, extract_roi 로직
│   ├── detectors.py       # test.py의 detect_pigmentation, pore, sebum 로직
...
├── engine.py              # 전체 파이프라인 실행 엔트리포인트 (analyze 함수)
└── config.yaml            # MAX_VALUES 및 알고리즘 하이퍼파라미터 설정
```

## 📊 Performance Validation
AI-Hub의 '한국인 안면 피부 데이터' 라벨링(JSON)과 측정치를 대조하여 검증 완료 <br>
색소 침착의 경우, 다른 항목 대비 오차율이 높음 -> 다른 데이터를 함께 테스트, 오차율 줄여나갈 것
